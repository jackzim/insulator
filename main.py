...
if __name__ == "__main__":
    ...
    if erms == True:
        ...
        erms_kit = [["Placeholder", 0]]
    
    print(f"PRM parts are {prm_list}")
    print(f"Adder Parts List: {adder_parts}")
    rotary_parts = rotary_adders(part_list)
    compressions = internal_wiring(amperage, one_line, voltage_code, enclosure_code)
    tmts_kit = full_tmts(amperage, voltage_code, part_list)
    full_list = chain(adder_parts, prm_list, breaker_kit, rotary_parts, erms_kit, tmts_kit)
...
## Excel Dumping
if purpose_validity and modifier_validity == True:
